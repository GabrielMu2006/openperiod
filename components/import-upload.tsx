  ...
  return (
    <div className="upload-flow">
      ...
      >
        ...
        <p>{file ? `${(file.size / 1024).toFixed(0)} KB · 可以开始解析` : "支持北京大学教务系统导出的课表，或下方标准模板；文件只用于解析，不会被永久保存。"}</p>
        <button type="button" onClick={() => inputRef.current?.click()}>{file ? "重新选择" : "选择 Excel 文件"}</button>
        <small>支持 .xlsx / .xls，最大 5MB</small>
      </div>
      {error && <p className="form-error" role="alert">{error}</p>}
      <div className="upload-actions"><a href="/OpenPeriod-PKU-Template.xlsx" download>下载标准模板（北大教务课表格式）</a><button type="button" disabled={!file || pending} onClick={upload}>{pending ? "正在解析…" : "解析并预览"}</button></div>
    </div>
  );
}
