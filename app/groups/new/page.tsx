...
function GroupFormPage({ title, description, children }: { title: string; description: string; children: React.ReactNode }) {
  return <main className="form-page"><section className="form-card"><a className="back-link" href="/groups">← 返回群组</a><p className="eyebrow">OPENPERIOD</p><h1>{title}</h1><p>{description}</p>{children}</section></main>;
}
