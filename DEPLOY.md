...
仓库版本为 `0.2.0`。截至 2026-09-20，本轮代码、`0007`–`0010` 迁移和部署模板仅在本地实现与验证，尚未部署，也没有对生产数据库执行迁移或其他写操作。

ICP 备案已于 2026-09-20 通过省通信管理局审核（备案号见阿里云 ICP 备案系统；通知建议 7 小时后访问，域名须在 30 天内解析到阿里云内地服务器，开通后 30 日内需办理公安备案）。下一步即执行「国内服务器与 HTTPS 边界」与「数据库迁移」两节的上线流程。
...
- 密码：15–128 字符，Argon2id（19 MiB、2 次迭代、并行度 1、随机盐）。
